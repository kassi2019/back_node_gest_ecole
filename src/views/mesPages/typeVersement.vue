  ...
  <div>
                  ...
                  <div>
                    <div class="table-responsive">
                      ...
                      <table class="table">
                        <thead class="thead-dark">
                          <tr>
                            ...
                            <th scope="col" style="width: 90%">Libelle</th>
                             <!-- <th scope="col" style="width: 90%">Montant versement</th> -->
                              <!-- <th scope="col" style="width: 90%">Date début</th>
                        ...
                        <tbody>
                          <tr
                            v-for="(item, index) in storetypeversement.gettertypeversement"
                            :key="item.id"
                          >
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <button
                ...
                @click.prevent="Enregistrementtypeversement"
              >
                Enregistrer
              </button>
            </div>
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Modifier type versement</h5>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
          ...
          <div class="modal-footer">
            ...
            <button
              type="button"
              class="btn btn-success"
              @click.prevent="modificationtypeversement"
            >
              Modifier
            </button>
          </div>
...
function Enregistrementtypeversement() {
  // $v.value.$touch();
  //if (!$v.value.$invalid) {
  try {
    let obj: any = {
    ...
    //   console.log(obj)
    storetypeversement.ajoutertypeversement(obj).then(() => {
      ViderChamps();
...
const showModalDecision = (id: number) => {
  const d_data = storetypeversement.gettertypeversement.find(
...
function modificationtypeversement() {
  //$v1.value.$touch();
  // if (!$v1.value.$invalid) {
  formmod.id = formmod.id;
  try {
    // isLoading.value = true;
    storetypeversement.modifiertypeversement(formmod).then(() => {
      //isLoading.value = false;
      // closeModal();
    });
...
function supprimer(id: any) {
  Swal.fire({
    ...
    if (res.isConfirmed) {
      storetypeversement.Supprimertypeversement(id);
    }
  });
}
onMounted(() => {
