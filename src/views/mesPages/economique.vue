  ...
  <div>
                  ...
                  <div>
                    <div class="table-responsive">
                      ...
                      <table class="table">
                        ...
                        <tbody>
                          <tr
                            v-for="(item, index) in storeeconomique.gettereconomique"
                            :key="item.id"
                          >
          ...
          <div class="modal-content">
            ...
            <div class="modal-footer">
              ...
              <button
                ...
                @click.prevent="Enregistrementeconomique"
              >
                Enregistrer
              </button>
            </div>
    ...
    >
      <div class="modal-dialog">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Modifier economique</h5>
            <button
              type="button"
              class="btn-close"
              data-bs-dismiss="modal"
          ...
          <div class="modal-footer">
            ...
            <button
              type="button"
              class="btn btn-success"
              @click.prevent="modificationeconomique"
            >
              Modifier
            </button>
          </div>
...
function Enregistrementeconomique() {
  // $v.value.$touch();
  //if (!$v.value.$invalid) {
  try {
    let obj: any = {
    ...
    //   console.log(obj)
    storeeconomique.ajoutereconomique(obj).then(() => {
      ViderChamps();
...
const showModalDecision = (id: number) => {
  const d_data = storeeconomique.gettereconomique.find(
...
function modificationeconomique() {
  //$v1.value.$touch();
  // if (!$v1.value.$invalid) {
  formmod.id = formmod.id;
  try {
    // isLoading.value = true;
    storeeconomique.modifiereconomique(formmod).then(() => {
      //isLoading.value = false;
      // closeModal();
    });
...
function supprimer(id: any) {
  Swal.fire({
    ...
    if (res.isConfirmed) {
      storeeconomique.Supprimereconomique(id);
    }
  });
}
onMounted(() => {
